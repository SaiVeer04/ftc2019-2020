...
@Autonomous(name = "SkystoneBlueLeft")
public class SkystoneBlueLeft extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        for (int i = 0; i <3; i++) {
            if (robot.colorSensor.red() == 0 && robot.colorSensor.blue() == 0 && robot.colorSensor.green() == 0) {
                ...
                robot.backwards(6, 0.7); //move backwards (DETERMINES LEFT OR RIGHT)
                break; //exit the loop
            } else {
        ...
        while (robot.bottomSensor.red() < 1) { //move to the other side by strafing right
            robot.fl.setPower(-1);
            robot.fr.setPower(-1);
            robot.bl.setPower(1);
            robot.br.setPower(1);
        ...
        robot.turn(90, "r", 0.5); //turn to drop the stone past the line
        ...
        for (int i = 0; i <3; i++) {
            if (robot.colorSensor.red() == 0 && robot.colorSensor.blue() == 0 && robot.colorSensor.green() == 0) {
                ...
                robot.intakeLeft.setPower(0);
                robot.intakeRight.setPower(0);
                robot.backwards(10, 0.7); //move backwards (THIS IS THE VALUE THAT DETERMINES LEFT OR RIGHT)
                break; //exit the loop
            } else {
